...
- **Solutions (provider-specific implementations of a case):**
  - [ ] `solutions/aws/fraud-detection-solution.md`
  - [ ] `solutions/azure/fraud-detection-solution.md`
...
├── 01-ml-system-design/
│   ├── notes.md
│   ├── cases/              ← provider-agnostic designs
│   └── solutions/          ← concrete implementations of a case, per cloud provider
│       ├── aws/
│       └── azure/
...
- **`cases/`** — completed design docs, provider-agnostic. This is the portfolio: what a recruiter or interviewer would actually open.
- **`solutions/<provider>/`** — concrete implementation of a case using a specific cloud's services (e.g. `solutions/aws/fraud-detection-solution.md`). Keeps abstract design reasoning separate from vendor-specific tool choices, and lets the same case be re-solved on multiple clouds.
